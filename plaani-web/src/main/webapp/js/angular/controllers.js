...
terminplanerApp.controller("TerminNeuController", ["$scope", "Termine", function ($scope, Termine) {
    $scope.termin = {};
    $scope.$watch("emailsRaw", function (emailsRaw) {
        if (typeof emailsRaw === "string" && emailsRaw) {
            $scope.termin.emails = emailsRaw.split(/[,;\s\r\n]+/);
        }
...
terminplanerApp.controller("UserLoginController", ["$scope", "$location", "$routeParams", "Login", "$resource",
    function ($scope, $location, $routeParams, Login, $resource) {
    /**
     * Login a user and update HTTP Auth Headers
     */
    $scope.submit = function () {
        var root = $resource(terminplanerApp.serviceUrl("/")).get(null, function () {
            var user = root.resource("login").put({username: $scope.username, password: $scope.password}, function () {
